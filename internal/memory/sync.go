...
// household space — `lore space invite` on one store and `lore join` on the others — is
// provisioned out of band by the operator, and deliberately so: lore's Go API exposes no
// invite and no join, and a household assistant minting its own memberships would be
// kenward taking a decision that is not its to take. See docs/IMPLEMENTATION.md §8 for
// the provisioning recipe. It is the one thing in this file's story that still wants the
// `lore` command, and it is a person's step rather than kenward's.
//
