...
func TestWizardWritesIdentityAndPersona(t *testing.T) {
	h := newHarness(t)
	runPersonaWizard(t, h, url.Values{
		"agents":            {"per_member"},
...
func TestWizardDefaultsToOneAssistant(t *testing.T) {
	h := newHarness(t)
	runPersonaWizard(t, h, url.Values{"update_channel": {"stable"}, "idle_timeout": {"0s"}})
	cfg := readWrittenConfig(t, h.configPath)
...
func TestSettingsEditsKenwardsPersonaAndKeepsMembers(t *testing.T) {
	...
	if after.Household.Agents != config.AgentsPerMember {
		t.Errorf("household.agents = %q, want per_member", after.Household.Agents)
	}
...
func runPersonaWizard(t *testing.T, h *harness, advanced url.Values) {
	t.Helper()
	token := h.issueToken()
