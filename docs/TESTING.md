...
- **The shared space in isolated mode, beyond one run.** It works now: every pod runs
  lore's sync daemon in its own process (D-044), and a three-pod household on real Podman
  read an entry written in one pod from the other two while every private entry stayed
